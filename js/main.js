var crop = function(item){
	...
	//生成选框上的各个点
	for(var i=0;i<8;i++){
		var dotDivs = document.createElement("div");
		dotDivs.setAttribute('class','minDiv');
		switch(i){
			case 0:
				dotDivs.id = 'left-up';
				break;
			...
			case 6:
				...
				break;
			case 7:
				dotDivs.id = 'right-down';
				break;
		}
		mainDiv.appendChild(dotDivs);
	}
	var ID = function(id){
		return document.getElementById(id);
	};
	
	var boxDiv = ID('main').parentNode;//外层容器
	var mainDiv = ID('main');//选择层
	var leftUpDiv = ID('left-up');//坐上角触点
	var leftDiv = ID('left');//左中间触点
	var leftDownDiv = ID('left-down');//左下角触点
	var upDiv = ID('up');//上中间触点
	var downDiv = ID('down');//下中间触点
	var rightUpDiv = ID('right-up');//右上角触点
	var rightDiv = ID('right');//右中间触点
	var rightDownDiv = ID('right-down');//右下角触点
